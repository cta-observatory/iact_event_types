...
if __name__ == "__main__":
    ...
    # IMPORTANT: Be aware that the gamma file goes first,
    # as it will be used to calculate the event type threshlds.
    dl2_file_list = [
    ...
    if "gamma" not in dl2_file_list[0]:
        raise ValueError(
            "The first DL2 file to analyze must be the gamma file, as we need it to comput the"
    ...
    for dl2_file in dl2_file_list:
        ...
        if "gamma" in dl2_file:
            ...
            # Calculate event types for proton and electron events,
            # using the same event type thresholds as in the gamma-like gammas:
